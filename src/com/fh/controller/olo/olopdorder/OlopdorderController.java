package com.fh.controller.olo.olopdorder;
...
/** 
 * 类名称：OlopdorderController
 * 创建人：FH 
 * 创建时间：2018-03-21
 */
@Controller
@RequestMapping(value="/olopdorder")
public class OlopdorderController extends BaseController {
	
	String menuUrl = "olopdorder/list.do"; //菜单地址(权限用)
	@Resource(name="olopdorderService")
	private OlopdorderService olopdorderService;
	...
	public void delete(PrintWriter out){
		logBefore(logger, "删除Olopdorder");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		try{
			pd = this.getPageData();
			olopdorderService.delete(pd);
			out.write("success");
			out.close();
		} catch(Exception e){
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, "修改Olopdorder");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		olopdorderService.edit(pd);
		mv.addObject("msg","success");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public ModelAndView goAdd(){
		logBefore(logger, "去新增Olopdorder页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			mv.setViewName("olo/olopdorder/olopdorder_edit");
	...
	public ModelAndView goEdit(){
		logBefore(logger, "去修改Olopdorder页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			pd = olopdorderService.findById(pd);	//根据ID读取
			mv.setViewName("olo/olopdorder/olopdorder_edit");
	...
	public Object deleteAll() {
		logBefore(logger, "批量删除Olopdorder");
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				olopdorderService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
	...
	public ModelAndView exportExcel(){
		logBefore(logger, "导出Olopdorder到excel");
