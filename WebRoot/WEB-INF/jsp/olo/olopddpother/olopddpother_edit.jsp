...
<body>
	<form action="olopddpother/${msg }.do" name="Form" id="Form" method="post" enctype="multipart/form-data">
		...
		<table id="table_report" class="table table-striped table-bordered table-hover">
			...
			<tr>
				...
				<td><input type="text" name="CODE" id="CODE" value="${pd.CODE}" maxlength="32" placeholder="这里输入编码" title="编码"/></td>
			</tr>
			<tr>
				<td style="width:70px;text-align: right;padding-top: 13px;">尺寸:</td>
				<td><input type="text" name="DIMENSION" id="DIMENSION" value="${pd.DIMENSION}" maxlength="32" placeholder="这里输入尺寸" title="尺寸"/></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>	<textarea name="REMARKS" id="REMARKS" maxlength="1000"  placeholder="这里输入说明">${pd.REMARKS}</textarea>
				<%-- <input type="text" name="REMARKS" id="REMARKS" value="${pd.REMARKS}" maxlength="32" placeholder="这里输入说明" title="说明"/> --%></td>
			</tr>
			<tr>
					<td style="width:70px;text-align: right;padding-top: 13px;">台面分类:</td>
					<td>
			...
			<tr style="display: none">
				...
				<td><input type="hidden" name="IMAGE_URL" id="IMAGE_URL" value="${pd.IMAGE_URL}" maxlength="32" placeholder="这里输入图片地址（列表展示）" title="图片地址（列表展示）"/></td>
			</tr>
			<tr>
				<td style="width:70px;text-align: right;padding-top: 13px;">图片:</td>
				<td>
				<input type="hidden" name="IMAGE_BUG_URL" id="IMAGE_BUG_URL" value="${pd.IMAGE_BUG_URL}" maxlength="32" placeholder="这里输入图片路径" title="图片路径"/>
			...
			<tr >
				...
				<td>
				<select  name="ISDETAILS" id="ISDETAILS" title="请是否有详情"  >
						<option value="0" <c:if test="${pd.ISDETAILS == '0' }">selected</c:if> >没有</option>
						<option value="1" <c:if test="${pd.ISDETAILS == '1'  }">selected</c:if> > 有</option>
					</select>
			<%-- 	<input type="text" name="ISDETAILS" id="ISDETAILS" value="${pd.ISDETAILS}" maxlength="32" placeholder="这里输入是否有详情 0 没有详情，1有详情" title="是否有详情 0 没有详情，1有详情"/> --%>
				</td>
			</tr>
			<tr >
				<td style="width:70px;text-align: right;padding-top: 13px;">生效时间:</td>
				<td><input class=" date-picker" type="text" name="EFFECTIVE_TIME" id="EFFECTIVE_TIME" value="${pd.EFFECTIVE_TIME}" maxlength="32" placeholder="这里输入生效时间（商品展示生效时间）" title="生效时间（商品展示生效时间）"/></td>
			</tr>
		<%-- 	<tr>
