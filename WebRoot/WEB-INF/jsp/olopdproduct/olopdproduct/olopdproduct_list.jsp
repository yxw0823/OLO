...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="olopdproduct/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
					...
					<c:if test="${QX.cha == 1 }">
						<td style="vertical-align:top;"><a class="btn btn-mini btn-light" onclick="inportImg();" title="导入图片"><i id="nav-search-icon" class=" icon-cloud-upload">导入图片</i></a></td>
						<td style="vertical-align:top;"><a class="btn btn-mini btn-light" onclick="inportExcel();" title="导入商品"><i id="nav-search-icon" class="icon-upload">导入商品</i></a></td>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										...
										<td><input  type="number" maxlength="10" id="${var.GOODS_ID}" class="sorting" value="${var.SORTING}" style="width: 30px"  /></td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>olopdproduct/goAdd.do';
			 diag.Width = 950;
			 diag.Height = 655;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>olopdproduct/delete.do?GOODS_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>olopdproduct/goEdit.do?GOODS_ID='+Id;
			 diag.Width = 950;
			 diag.Height = 655;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>olopdproduct/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>olopdproduct/excel.do';
		}
		...
		function inportExcel(){
			 ...
			 diag.Drag=true;
			 diag.Title ="批量上传";
			 diag.URL = '<%=basePath%>olopdproduct/goInportExcel.do';
			 diag.Width = 350;
			 diag.Height = 100;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function inportImg(){
			 ...
			 diag.Drag=true;
			 diag.Title ="批量上传";
			 diag.URL = '<%=basePath%>olopdproduct/goInputrtImages.do';
			 diag.Width = 350;
			 diag.Height = 100;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
